<form id="teacherRegistrationForm" method="POST">
    ...
    <div class="form-group">
        <label for="teacher_full_name" class="form-label">
    ...
    <div class="form-group">
        <label for="teacher_id" class="form-label">
            <i class="fas fa-id-badge"></i> Instructor ID Number *
        </label>
        <input 
            type="text" 
            id="teacher_id" 
            ...
            title="Instructor ID must contain only numbers, or exactly 2 capital letters followed by numbers" 
            oninput="validateInstructorId(this)"
            required
        >
    </div>
    
    <style>
        .form-input.valid-two-letters {
            border-color: #22c55e !important;
            background-color: #f0fdf4 !important;
            box-shadow: 0 0 0 3px rgba(34, 197, 94, 0.1) !important;
        }
        .form-input.valid-two-letters:focus {
    ...
    <script>
        function validateInstructorId(input) {
            ...
            input.classList.remove('valid-two-letters');
            
            // Add green styling if exactly 2 letters are entered
            if (hasTwoLetters) {
                input.classList.add('valid-two-letters');
            }
        }
        
        // Clear any old validation messages on page load
        document.addEventListener('DOMContentLoaded', function() {
            // Clear any existing validation messages
            const validationMessages = document.querySelectorAll('.validation-message');
            validationMessages.forEach(msg => {
                if (msg.textContent.includes('Name must contain only letters and spaces')) {
                    msg.remove();
                }
            });
        });
    ...
    <div class="form-group">
        <label for="teacher_department" class="form-label">
            <i class="fas fa-building"></i> Department *
        </label>
        <select id="teacher_department" name="department" class="form-input" required>
    ...
    <div class="form-group">
        ...
        <div class="password-input-wrapper">
            <input type="password" id="teacher_password" name="password" class="form-input" 
        ...
        <div class="password-strength-meter" id="teacher_password_strength" style="display: none;">
        ...
        <div class="password-requirements" id="teacher_password_requirements" style="display: none;">
    ...
    <div class="form-group">
        <label for="teacher_confirm_password" class="form-label">
            <i class="fas fa-lock"></i> Confirm Password *
        </label>
        <div class="password-input-wrapper">
            <input type="password" id="teacher_confirm_password" name="confirm_password" class="form-input" 
                   pattern="^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$" 
        ...
        <div class="validation-message" id="teacher_confirm_password_message" style="display: none;"></div>
    </div>

    <div class="alert alert-warning">
        <i class="fas fa-clock"></i>
        <strong>Teacher Registration:</strong> Your account will require admin approval before you can log in.
    </div>

    <div class="form-group">
        <button type="submit" class="btn btn-primary btn-block" onclick="handleTeacherRegistration(event)">
    ...
    <div class="modal-content">
        ...
        <div class="modal-body">
            <div class="success-message">
                <p><strong>Your teacher account has been created successfully!</strong></p>
                ...
                <div class="info-box">
                    ...
                    <ul>
                        ...
                        <li>You will be redirected to your Teacher Dashboard</li>
                    </ul>
                </div>
...
<script>
async function checkDuplicateTeacherId(teacherId) {
    try {
        const res = await fetch('api/check-duplicate-id.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ id: String(teacherId || '').trim(), role: 'teacher' })
        });
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    teacherIdInput?.addEventListener('blur', async function() {
        const message = await checkDuplicateTeacherId(this.value);
        if (message) {
            alert(message);
            this.focus();
        }
...
function handleTeacherRegistration(event) {
    ...
    checkDuplicateTeacherId(teacherId).then((message) => {
        if (message) {
            alert(message);
